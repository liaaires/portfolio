...
export default function Home() {
  return (
    <div className="bg-white relative min-h-screen w-full" data-name="Home">
     ...
     <div id="projetos-section" className="absolute bg-[#190a46] h-[750px] left-0 overflow-clip top-[750px] w-full flex items-center justify-center">
        
        {/* Título da seção isolado no canto superior esquerdo */}
        <p className="absolute font-bold leading-[normal] left-[128px] not-italic text-[40px] text-white top-[50px] whitespace-pre-wrap" style={{ fontFamily: 'Inter, sans-serif' }}>Projetos</p>

        {/* CONTÊINER FLEX: Centraliza o Notebook e o Texto juntos no meio da tela */}
        <div className="flex flex-col lg:flex-row items-center justify-center gap-16 lg:gap-24 w-full max-w-[1200px] z-10">
          
          {/* BLOCO DA IMAGEM: Círculo Roxo + Notebook */}
          <div className="relative flex items-center justify-center w-[633px] h-[586px] shrink-0">
            {/* Círculo Roxo ao fundo */}
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              <svg width="586" height="586" viewBox="0 0 586 586" fill="none">
                <circle cx="293" cy="293" r="293" fill="#C27CFA" />
              </svg>
            </div>
            
            {/* Notebook (Posição Relative para ficar no centro do bloco) */}
            <div className="relative h-[365px] w-[633px] z-10" data-name="Devices / MacBook Pro">
          ...
          {/* BLOCO DO TEXTO */}
          <div className="flex flex-col gap-[16px] items-start shrink-0 z-10">
            <p className="font-bold leading-[normal] not-italic text-[50px] text-white w-[459px] whitespace-pre-wrap m-0" style={{ fontFamily: 'Inter, sans-serif' }}>Instituto Universidade Virtual</p>
            <p className="font-normal leading-[normal] not-italic text-[20px] text-white m-0" style={{ fontFamily: 'Inter, sans-serif' }}>UI/UX Design</p>
            <Link to="/casestudy/instituto-universidade-virtual" onClick={() => window.scrollTo(0, 0)} className="bg-[#f26a2c] flex h-[45px] items-center justify-center px-[20px] py-[10px] rounded-[20px] no-underline mt-4 hover:bg-[#d95a22] transition-colors duration-200">
              <p className="font-normal leading-[normal] not-italic text-[14px] text-white m-0" style={{ fontFamily: 'Inter, sans-serif' }}>Leia Mais</p>
            </Link>
          </div>

        </div>
      </div>
    {/* SEÇÃO OBRIGADA & FOOTER */}
      <div className="absolute top-[2250px] left-0 w-full h-[600px] flex flex-col justify-between px-[128px] pt-32 pb-10">
        
        {/* Bloco de Texto (AGORA ESTICANDO ATÉ A MARGEM DIREITA) */}
        {/* Removido o max-w-[1000px] para a div crescer livremente */}
        <div className="flex flex-col items-start gap-10 w-full">
          <p className="font-bold leading-[normal] not-italic text-[70px] text-black m-0" style={{ fontFamily: 'Inter, sans-serif' }}>
            Obrigada!
          </p>
          
          {/* Adicionado w-full no parágrafo para ele ocupar todo o espaço */}
          <p className="font-extralight leading-[35px] not-italic text-[#0a0614] text-[18px] m-0 w-full whitespace-pre-wrap" style={{ fontFamily: 'Inter, sans-serif' }}>
          ...
          <Link to="/about" onClick={() => window.scrollTo(0, 0)} className="no-underline">
            <BackgroundImageAndText text="Saiba mais sobre" additionalClassNames="h-[60px] w-[250px]" />
          </Link>
        </div>

        {/* FOOTER (Linha, Copyright à esquerda e LinkedIn à direita) */}
        <div className="w-full border-t border-[#CCCCCC] pt-6 flex justify-between items-center mt-20">
          <p className="font-thin leading-[normal] not-italic text-[#888] text-[12px] m-0" style={{ fontFamily: 'Inter, sans-serif' }}>
            {`©2026 Lia Aires`}
          </p>
...
{/* SEÇÃO CATÁLOGO RADIANT PATTERNS */}
      <div className="absolute top-[1500px] left-0 w-full h-[750px] flex items-center overflow-clip" style={{ backgroundImage: "url('data:image/svg+xml;utf8,<svg viewBox=\\'0 0 1440 750\\' xmlns=\\'http://www.w3.org/2000/svg\\' preserveAspectRatio=\\'none\\'><rect x=\\'0\\' y=\\'0\\' height=\\'100%\\' width=\\'100%\\' fill=\\'url(%23grad)\\' opacity=\\'1\\'/><defs><radialGradient id=\\'grad\\' gradientUnits=\\'userSpaceOnUse\\' cx=\\'0\\' cy=\\'0\\' r=\\'10\\' gradientTransform=\\'matrix(4.4087e-15 37.5 -72 2.2962e-15 720 375)\\'><stop stop-color=\\'rgba(255,250,249,1)\\' offset=\\'0\\'/><stop stop-color=\\'rgba(224,213,209,0.5)\\' offset=\\'1\\'/></radialGradient></defs></svg>')" }}>
        
        {/* Fundo Desfocado (Blur) */}
        <div className="absolute inset-0 bg-[rgba(255,255,255,0.3)] blur-[25px] pointer-events-none" />
        
        {/* Contêiner Flexível para alinhar Texto à esquerda e Imagem à direita */}
        <div className="relative z-10 w-full px-[128px] flex flex-col lg:flex-row justify-between items-center gap-16">
          
          {/* TEXTO (Alinhado à Margem Esquerda) */}
          <div className="flex flex-col gap-[16px] items-start shrink-0 text-[#0a0614]">
            <div className="font-bold text-[50px] leading-[1.1] whitespace-nowrap" style={{ fontFamily: 'Inter, sans-serif' }}>
              <p className="m-0">Catálogo Radiant</p>
              <p className="m-0">Patterns</p>
            </div>
            <p className="font-normal text-[20px] m-0" style={{ fontFamily: 'Inter, sans-serif' }}>UI Design</p>
            <p className="font-normal text-[20px] m-0" style={{ fontFamily: 'Inter, sans-serif' }}>Página em construção</p>
          </div>

          {/* IMAGEM + CÍRCULO AMARELO (Alinhados à Margem Direita) */}
          <div className="relative flex items-center justify-center shrink-0 w-[676px] h-[496px]">
            
            {/* Círculo Amarelo (Fundo - Agora Centralizado!) */}
            {/* Removido o translate-x-[45px] e translate-y-[37px] */}
            <div className="absolute flex items-center justify-center pointer-events-none">
              <svg width="586" height="586" viewBox="0 0 586 586" fill="none">
                <circle cx="293" cy="293" r="293" fill="#FEDE64" />
              </svg>
            </div>
            
            {/* Caixa da Imagem (Frente) */}
            <div className="relative h-full w-full rounded-[24px] pointer-events-none z-10">
              <div className="absolute inset-0 overflow-hidden rounded-[24px]">
                <img alt="Catálogo Radiant Patterns" className="absolute h-[387.6%] left-[-0.08%] max-w-none top-[-0.05%] w-full" src={imgRectangle3124} />
              </div>
